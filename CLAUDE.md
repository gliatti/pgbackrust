...
- `src/build/` (the C code generator that emits `config.auto.*`, `parse.auto.c.inc`, `help.auto.*`, error tables, PG interface tables) is being replaced by `crates/pgbr-build`, producing the same output paths so the existing test harness keeps working during the transition.
