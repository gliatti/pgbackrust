...
//! process-global log lock (`super::log_lock`) for their whole body, serialising every
//! access to the scratchpad and to the `LogState` fields read during a dispatch. The same
//! lock guards the parent module's configuration setters, so a setter cannot tear a field
//! mid-dispatch. [`log_signal`] is the lone exception: it runs from a signal handler at
//! process death, where locking a `Mutex` is not async-signal-safe, so it stays lock-free
//! and best-effort (as the C original was).
//!
//! # Errors
//!
...
/// # Safety
///
/// The caller must guarantee exclusive access. Both message entry points hold the
/// process-global log lock (`super::log_lock`) for the duration, so only one renders at a
/// time; [`log_signal`] runs only at signal-driven process death. Two concurrent emissions
/// would alias this `&mut`, which is undefined behaviour — hence the lock.
...
pub fn log_internal(
    ...
    // Serialise the whole render+dispatch against the shared scratchpad / LogState. The
    // same lock guards the parent module's config setters, so a setter cannot tear a field
    // we read mid-dispatch.
    let _emit = super::log_lock();
